...
int main(void) {
    ...
    const size_t testCount = sizeof(testStrings) / sizeof(TestData);
    ...
    for (i = 0; i < testCount; i++) {
        ...
        const int r1 = convertUtf8ToCp1251(test.utf8, cp1251);
        const int r2 = strcmp(cp1251, test.cp1251);

        char * result = "ПРОВАЛЕН :(";
        const bool success = (r1 == cp1251Size - 1) && (r2 == 0);
        ...
        if (success == false) {
            printf("\"Эталон\" | \"Результат преобразования\"\n");
            for (i = 0; i < strlen(test.cp1251) && i < strlen(cp1251); i++) {
                printf("%02hhX | %02hhX\n", test.cp1251[i], cp1251[i]);
            }
        }
